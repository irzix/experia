...
def test_loaders_read_current_project_and_release_evidence():
    assert load_project_version(ROOT / "pyproject.toml") == "0.8.1"
    releases = load_changelog_releases(ROOT / "CHANGELOG.md")
    current = next(release for release in releases if release.version == "0.8.1")
    assert current.release_date == "2026-08-02"
